...
describe("v2 — admin views", () => {
  it("GET /api/admin/v2/modules + /tools + /tool-calls", async () => {
    ...
    try {
      ...
      const modulesRes = await fetch(`${server.url}/api/admin/v2/modules`, { headers: adminHeaders });
      ...
      const toolsRes = await fetch(`${server.url}/api/admin/v2/tools`, { headers: adminHeaders });
      ...
      const callsRes = await fetch(`${server.url}/api/admin/v2/tool-calls`, { headers: adminHeaders });
      ...
      const filteredRes = await fetch(
        `${server.url}/api/admin/v2/tool-calls?tool=framework.tasks.create`,
