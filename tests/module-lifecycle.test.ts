...
describe("v2 — lifecycle + install state", () => {
  it("admin install/uninstall + dispatcher gate + lazy install for default modules", async () => {
    ...
    try {
      ...
      const installs1 = await fetch(`${server.url}/api/admin/installs`, { headers: adminHeaders });
      ...
      const installRes = await fetch(`${server.url}/api/admin/modules/beta/install`, {
        method: "POST",
      ...
      const uninstallRes = await fetch(`${server.url}/api/admin/modules/beta/uninstall`, {
        method: "POST",
        headers: adminPostHeaders,
        body: "{}",
      });
      ...
      const installs2 = await fetch(`${server.url}/api/admin/installs`, { headers: adminHeaders });
      ...
      const ghost = await fetch(`${server.url}/api/admin/modules/ghost/install`, {
        method: "POST",
        headers: adminPostHeaders,
        body: "{}",
      });
  ...
  it("re-installing an already-installed module is idempotent (no duplicate row, hooks re-fire)", async () => {
    ...
    try {
      ...
      const installPath = `${server.url}/api/admin/modules/noop/install`;
