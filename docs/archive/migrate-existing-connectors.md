# Migrating Existing Connectors to the Right Architecture (HISTORICAL)

> ⛔ **Historical / superseded.** This doc planned a migration to
> the v1 `@boringos/connector-sdk` manifest contract. That SDK
> was never published — instead the framework collapsed
> connector + app + plugin into the single Module shape. The
> built-in connectors (`connector-google`, `connector-slack`)
> have already been ported. New work should follow
> [`BUILD-A-MODULE.md`](../../BUILD-A-MODULE.md) directly.

**Status:** Plan, not executed; superseded by the Module collapse.
**Audience:** Historical reference only.
**Read first:** [`BUILD-A-MODULE.md`](../../BUILD-A-MODULE.md), [`MODULES.md`](../../MODULES.md).
