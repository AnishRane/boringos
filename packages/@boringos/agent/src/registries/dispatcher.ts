...
//
// tool dispatcher — the single code path for invoking any
// registered Tool.
//
// Two callers:
...
async function finish<T>(
  ...
  if (deps.db) {
    try {
      ...
      console.error(
        `[tool-dispatch] failed to persist tool_calls row for ${fullName}:`,
        auditError,
      );
    }
  }
...
export async function invoke<TInput = unknown, TOutput = unknown>(
  ...
  if (
    ...
    throw new Error(
      `[tool-dispatch] internal error invoking "${fullName}": ${dispatched.result.error.message}`,
    );
  }
