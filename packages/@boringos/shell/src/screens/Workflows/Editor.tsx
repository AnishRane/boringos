...
export function Editor({ auth, workflow, tools, onSaved }: EditorProps) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* Header */}
      <header className="flex items-center gap-3 px-5 py-2 border-b border-slate-100 bg-white">
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          className="text-[15px] font-semibold text-slate-900 bg-transparent border-0 focus:outline-none focus:ring-0 w-72 truncate"
        />
        ...
        <button
          ...
          className="text-[11px] text-slate-500 hover:text-slate-900 px-2 py-1 rounded border border-slate-200 hover:border-slate-300 flex items-center gap-1"
          title="Cmd-K"
        >
          <span>+ Block</span>
          <kbd className="font-mono text-[9px] text-slate-400">⌘K</kbd>
        </button>
        <button
          type="button"
          onClick={onRun}
          className="text-[11px] font-medium text-white bg-slate-900 hover:bg-slate-800 px-2.5 py-1 rounded"
        >
          ▶ Run
        </button>
      </header>
      ...
      {TABS.map((t) => (
        <button
          ...
          className={`px-2.5 py-1 rounded transition-colors ${
            current === t.id ? "text-slate-900 font-medium bg-slate-100" : "text-slate-500 hover:text-slate-900"
          }`}
        >
          {t.label}
        </button>
...
function SourceTab({
  ...
  return (
    <div className="flex-1 p-5 overflow-auto bg-white">
      ...
      <textarea
        rows={28}
        className="w-full h-full rounded border border-slate-200 px-3 py-2 text-[12px] font-mono text-slate-800 focus:outline-none focus:border-slate-400"
        spellCheck={false}
        value={text}
        onChange={(e) => {
