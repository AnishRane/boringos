  ...
  return (
    <div className="space-y-4 max-w-4xl">
      <div className="flex items-baseline justify-between">
        <h2 className="text-base font-medium text-slate-900">Tool calls</h2>
        <button
          type="button"
          onClick={refresh}
        ...
        <div className="rounded-md border border-slate-200 divide-y divide-slate-100 bg-white">
          {calls.map((c) => (
            <div key={c.id} className="px-4 py-2.5 text-xs">
              <div className="flex items-baseline justify-between">
                <code className="text-slate-900">{c.toolName}</code>
                <span
                  className={`rounded px-1.5 py-0.5 text-[10px] uppercase tracking-wide ${
                    STATUS_BADGES[c.status] ?? "bg-slate-100 text-slate-600"
                  }`}
                >
                  {c.status}
                </span>
              </div>
              <div className="mt-1 flex gap-3 text-slate-500 text-[11px]">
                ...
                {c.runId && (
                  <span>
                    run <code className="text-slate-400">{c.runId.slice(0, 8)}</code>
                  </span>
                )}
              </div>
            </div>
